...
export default class ComUpdatePage {
  ...
  subModulesSelect: ElementFinder = element(by.css('select#com-subModules'));
  ...
  async subModulesSelectLastOption() {
    await this.subModulesSelect.all(by.tagName('option')).last().click();
  }

  async subModulesSelectOption(option) {
    await this.subModulesSelect.sendKeys(option);
  }

  getSubModulesSelect() {
    return this.subModulesSelect;
  }

  async getSubModulesSelectedOption() {
    return this.subModulesSelect.element(by.css('option:checked')).getText();
  }
  ...
  async enterData() {
    ...
    await this.subModulesSelectLastOption();
    await this.save();
