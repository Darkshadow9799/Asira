...
export default class SpeUpdatePage {
  ...
  speOrgIdInput: ElementFinder = element(by.css('input#spe-speOrgId'));
  ...
  async setSpeOrgIdInput(speOrgId) {
    await this.speOrgIdInput.sendKeys(speOrgId);
  }

  async getSpeOrgIdInput() {
    return this.speOrgIdInput.getAttribute('value');
  }
