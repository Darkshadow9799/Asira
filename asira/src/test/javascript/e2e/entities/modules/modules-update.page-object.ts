...
export default class ModulesUpdatePage {
  ...
  mSpeIdInput: ElementFinder = element(by.css('input#modules-mSpeId'));
  mProjIdInput: ElementFinder = element(by.css('input#modules-mProjId'));
  ...
  async setMSpeIdInput(mSpeId) {
    await this.mSpeIdInput.sendKeys(mSpeId);
  }

  async getMSpeIdInput() {
    return this.mSpeIdInput.getAttribute('value');
  }

  async setMProjIdInput(mProjId) {
    await this.mProjIdInput.sendKeys(mProjId);
  }

  async getMProjIdInput() {
    return this.mProjIdInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setMSpeIdInput('5');
    await waitUntilDisplayed(this.saveButton);
    await this.setMProjIdInput('5');
    await waitUntilDisplayed(this.saveButton);
